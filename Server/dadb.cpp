...
void Database::seed() {
    QString ins;
    ins += "INSERT OR IGNORE INTO users(id,name,role) VALUES";
    ins += "('d001','张医生','doctor'),";
...
QString Database::queryUserRole(const QString &userId, QString *nameOut) {
    QSqlQuery q(db_);
    q.prepare("SELECT name, role FROM users WHERE id = ?");
...
bool Database::createOrder(const QString &patientId,
                           ...
                           QString *err) {
    QSqlQuery q(db_);
    q.prepare("INSERT INTO orders(patient_id,doctor_id,dept,content,created_at) "
              "VALUES(?,?,?,?,?)");
...
int Database::countOrders(const QString &patientId,
                          ...
                          const QString &dateFromIso,
                          const QString &dateToIso) {
    QString sql = "SELECT COUNT(*) FROM orders";
...
QString Database::listOrdersAsJson(const QString &patientId,
    ...
    QString sql = "SELECT id, patient_id, doctor_id, dept, content, created_at FROM orders";
    QList<QVariant> binds;
    buildWhere(sql, binds, patientId, keyword, dept, dateFromIso, dateToIso);
...
bool Database::updateOrder(int id, const QString &dept, const QString &content, QString *err) {
    QSqlQuery q(db_);
    q.prepare("UPDATE orders SET dept=?, content=? WHERE id=?");
    q.addBindValue(dept);
    q.addBindValue(content);
    q.addBindValue(id);
    if (!q.exec()) {
